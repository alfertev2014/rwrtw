...
This document contains description of core ideas and terms to understand the library's runtime behavior and some implementation details.

The next chapter describes convenient DSL to build and compose UI components in declarative way.
...
_DOM_ is a representation of an HTML-document describing structure and logic of GUI elements in browser tab. The browser renders one _Live DOM_ for every tab. JavaScript code can create many _Dangling DOM_ nodes for inserting into the Live DOM later. The Live DOM is a kind of config of what the browser should show on a display. Live DOM describes the desirable state of GUI at one moment.

_Web engine_ is a browser's internal software component responsible to show and handle user interactions according to current state of the Live DOM in a tab. An application inserts and changes fragments of Live DOM. JavaScript code can:
...
Instantiating a component is just calling its factory function with special arguments. The first argument determines the place in the DOM tree where the component should insert its content. The second argument is a context of component rendering that represents lifecycle hooks.
...
A component can create its own Dangling DOM fragment and insert it in the Dangling DOM nodes of the parent component. The _Root component_ inserts the root Dangling DOM containing all rendered fragments of all child components into the Live DOM. Also, the root component is responsible to remove the inserted DOM-fragment from Live DOM if needed.

Every controller has a _Lifecycle_ with these phases (hooks):
...
- dispose - final cleanup actions, after which the controller object is not usable anymore

Controller can pass render and dispose phases only once, but it can be mounted again after unmount. The goal of the separate stage for mounting is caching or reusing components' DOM fragments with pre-initialized and bound controllers.

_Component tree_ is a tree of component instances (controllers) built along with DOM tree during rendering phase. A component can implement logic for conditional rendering of fragments of such Component trees. So the component is responsible to handle lifecycle of child controllers.
...
DOM API only allows inserting or removing DOM nodes relative to some neighbor nodes or a parent node. So, a controller of a component should remember where it is responsible to insert or remove its DOM content. Also, a component should know whether the adjacent nodes are managed by other controllers.

The idea is that a component should know and remember the place in which it is allowed to insert or remove DOM nodes. Every component builds some Dangling DOM fragment that is to be inserted into some place inside the other DOM fragment or Live DOM of the current document.
...
To remove a chain of nodes from DOM, we need the place where it starts and the place where it ends. After the removing, we need to remember the place where we can insert nodes again. This is the idea of _Placeholder_ - a special object to handle manipulations with DOM structure at one point in the DOM tree.

_Content_ of placeholder is zero or many DOM nodes and other placeholders. Placeholders inside content of a placeholder is _Child placeholders_. Child placeholders could be inside child Elements of content. _Placeholder tree_ is a tree of placeholders bound to DOM tree.
...
Placeholders could follow each other in a chain. Every placeholder must remember its place where it was created. After a placeholder is created, it cannot modify its place or move to another place. The only exception is when a placeholder is used as an item of a dynamic list.
...
- If a place is a Placeholder:
  ...
  - If the content of the Placeholder is empty - the DOM-place after the placeholder is the DOM-place of the place where this placeholder was created. It could be some other Placeholder or parent Placeholder, so we need to calculate recursively.
...
This algorithm targets to a chain of placeholders, with nested placeholders at start or end of parent placeholders. If a Placeholder is surrounded with regular DOM Nodes and contains "static" DOM Nodes, the bounds of its content in the DOM tree is clear.

## Dynamic list

When we need to represent a dynamic collection of items in DOM, we would need a collection of placeholders. _Dynamic list_ is an ordered collection of placeholders. Every placeholder in a dynamic list is created at place after previous placeholder. The first placeholder in a list is created at the place where the list was created. Dynamic list behaves as placeholder in calculation of DOM-places. So, dynamic list can be used everywhere placeholder can, but it has different set of operations.
...
When dynamic list performs the operations, it maintains places of the placeholders so they are organized in a linked list.
